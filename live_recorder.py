...
class LiveRecoder:
    ...
    def is_disabled_now(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                f'{self.flag} disable_hours 格式错误："{field}"，忽略禁用时段'
            )
            return False
        ...
        while True:
            try:
                ...
                if self.is_disabled_now():
                    logger.info(
                        f"{self.flag} 处于禁用时段（{self.disable_hours}），跳过检测"
                    )
    ...
    def stream_writer(self, stream, url, filename):
        ...
        try:
            ...
            elif re.search("SSL: CERTIFICATE_VERIFY_FAILED", str(error)):
                logger.warning(
                    f"{self.flag}SSL错误，将取消SSL验证：{filename}\n{error}"
                )
                self.ssl = False
            elif re.search("SSL: CERTIFICATE_VERIFY_FAILED", str(error)):
                logger.warning(
...
class Douyu(LiveRecoder):
    ...
    async def get_js(self):
        ...
        crypto_js = (await self.request(method="GET", url=self.crypto_js_url)).text
        ...
        if url not in recording:
            ...
            try:
                    ...
                    if data["status"] == 2:
                        ...
                        for quality_code in ("origin", "uhd", "hd", "sd", "md", "ld"):
                            if quality_data := stream_data["data"].get(quality_code):
                                ...
                                break
                        stream = HTTPStream(
                            self.get_streamlink(), live_url
                        )  # HTTPStream[flv]
                        await asyncio.to_thread(
                            self.run_record, stream, url, title, "flv"
                        )
            except Exception as e:
                logger.warning(
                    f"{self.flag} 接口解析失败，尝试使用 Streamlink 直接录制：{e}"
                )
                ...
                try:
                    ...
                    title = "streamlink"
                    await asyncio.to_thread(
                        self.run_record, best_stream, url, title, "flv"
                    )
                ...
                if url not in recording:
                    ...
                    if self.whitewords == "":
                        ...
                        if pattern.search(title):
                            ...
                            match = 0
                            logger.info(
                                f"标题不包含白名单内容:{self.whitewords}，暂不录制"
                            )
                    if self.blackwords == "":
                        match = match + 1
                    else:
                        ...
                        if pattern.search(title):
                            pass
                            logger.info(
                                f"标题包含黑名单内容:{self.blackwords}，暂不录制"
                            )
                        else:
                            match = match + 1
                    if match == 2:
        ...
        if url not in recording:
            response = (
                await self.request(
                    ...
                    data={"room_slug": self.id},
                )
            ).json()
            if response["room_status"] == "public":
                title = self.id
                streams = HLSStream.parse_variant_playlist(
                    session=self.get_streamlink(), url=response["url"]
                )
